...
function SourceIcon({ source }) {
  ...
  if (source === "notion")
    return (
      <span className={`${cls} text-[#e2e8f0]`} title="Notion">
        <FileText size={14} />
      </span>
    );
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-1.5">
            <h3 className="text-[13px] font-medium text-[#e2e8f0] leading-snug truncate">
              {item.item}
            </h3>
          ...
          <div className="flex items-center gap-3">
            <span className="font-mono text-[10px] text-[#475569]">
              Last visited:{" "}
              <span className="text-[#94a3b8]">{timeSince(item.last_touched)}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <div className="p-6 space-y-5 fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => refetch()}
          disabled={loading}
          className="p-2 rounded-lg bg-[#0f0f1a] border border-[#1e1e32] text-[#475569] hover:text-[#f59e0b] hover:border-[#f59e0b33] transition-all disabled:opacity-40"
        >
      ...
      {loading && !data ? (
        ...
        failedSources?.length > 0 ? (
          <p className="text-[13px] text-[#475569] text-center py-8 fade-in">
